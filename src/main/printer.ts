 ...
 */
function resolvePrinter(type: DocumentType): { name: string; autoPrint: boolean } {
...
export async function printDocument(job: PrintJobRequest): Promise<{ printer: string }> {
  ...
  try {
    ...
    await print(tempFile, {
      ...
      // These options work for most printers
      scale: 'noscale',  // Don't scale - print at actual size
    ...
    console.log(`[Printer] Job ${job.jobId} sent to printer successfully`);
